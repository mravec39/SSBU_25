...
Použil som Random Forest a ako parametre som použil n_estimators(počet stromov), max_dept(maximálna hĺbka stromu) a min_samples_split(minimálny počet vzoriek pre rozdelenie uzla)
...
Matthewsov korelačný koeficient (MCC) - Je to metrika, ktorá sa používa na hodnotenie klasifikačných modelov. 
...
Zaver z matic 
- Logistic Regression robí menej chýb celkovo (nižšie FP a FN).
- Random Forest má mierne vyššiu schopnosť správne klasifikovať triedu 1 (vyššie TP), ale za cenu vyšších chýb v iných oblastiach.

Zaver z grafov 
- Logistická regresia je v tomto prípade presnejšia aj stabilnejšia než Random Forest.
- Aj podľa MCC metriky je Logistická regresia lepšia. Má nielen vyšší priemer, ale aj výrazne menšiu variabilitu v jednotlivých replikáciách.
